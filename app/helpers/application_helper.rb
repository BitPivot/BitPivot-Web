module ApplicationHelper
  ...
  # Sets (if passed arguemnt) or gets a page title
  # Defaults should be set in config/app_config.yml
  #
  def title(new_title = nil)
    if new_title.nil?
      return content_for?(:title) ? content_for(:title) : APP_CONFIG['default_page_title']
    else
  ...
  def meta_description(new_description = nil)
    if new_description.nil?
      return content_for?(:meta_description) ? content_for(:meta_description) : APP_CONFIG['default_meta_description']
    else
  ...
  def meta_keywords(new_keywords = nil)
    if new_keywords.nil?
      return content_for?(:meta_keywords) ? content_for(:meta_keywords) : APP_CONFIG['default_meta_keywords']
    else
